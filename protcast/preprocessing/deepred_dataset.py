...
class DeepredDataset:
        ...
        for level, go_terms_level in go_terms_tree.items():
            ...
            for bucket, go_terms in go_terms_level.items():
                ...
                number_of_models = max(
                    1,
                    number_of_go_terms // self.go_terms_per_submodel,
                )
    ...
    def _build_submodel_dataset(
        ...
        X = np.zeros(
            (
                len(y_dict),
                FEATURE_VECTOR_LENGTHS[self.feature_vector_name],
            )
        )
    ...
    def summary(self):
        ...
        (
            p_bp_go_terms_tree,
            p_bp_go_terms_tree_lengths,
        ) = summarize_tree(self.bp_go_terms_tree)
        (
            p_cc_go_terms_tree,
            p_cc_go_terms_tree_lengths,
        ) = summarize_tree(self.cc_go_terms_tree)
        (
            p_mf_go_terms_tree,
            p_mf_go_terms_tree_lengths,
        ) = summarize_tree(self.mf_go_terms_tree)
