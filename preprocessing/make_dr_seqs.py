...
parser.add_argument("--outformat", default="swiss",
                    help="Output sequence file format")
parser.add_argument("-t", "--threshold", default=0.1,
                    type=float, help="Similarity threshold")
...
def main():
    builder = MakeDRSeqs(args.seqfile, args.informat, args.outformat, args.threshold,
...
class MakeDRSeqs:
    ...
    def __init__(self, seqfile, informat, outformat, threshold,
    ...
    def write_seqs(self, similar_seqs):
        ...
        if self.verbose:
            print("Output file '{0}' has {1} sequences".format(self.output, (len(seq_dict.keys()) - len(similar_seqs))))
