...
def train_multilabel(
    ...
    # Scale single-encoder inputs when requested. The dual-encoder path already
    # standardises its ESM/FV blocks above, so scaling again there would
    # double-scale — hence the `and not use_dual_encoder` guard.
    classifier = MultiLabelClassifier(
        ...
        go_dag=go_dag,
        random_state=seed,
        scale_features=(scale_features and not use_dual_encoder),
    )
    classifier.run()
