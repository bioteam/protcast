"""
run_knn_inference_embeds.py

Perform KNN multi-label inference using a model trained with ESM-C embeddings.

Same output format as run_multilabel_inference_embeds.py:
    Protein  GO_ID  GO_Name  Namespace  Score  Confidence

Example usage:

python3 scripts/run_knn_inference_embeds.py \
    -m model_knn.joblib \
    -g model_knn_GOEncoder.pkl \
...
def main():
    parser = argparse.ArgumentParser(
        description="KNN multi-label inference using ESM-C embeddings"
    )
    parser.add_argument(
        "-m", "--model_file", required=True, help="Path to model file (.joblib)"
    )
    parser.add_argument(
        "-g",
        "--goencoder_file",
    ...
    if args.verbose:
        print("Loading KNN model and GOEncoder...")
    artifact = KNNClassifier.load_model(args.model_file)
    go_decoder = GOEncoder.load(args.goencoder_file)

    if args.verbose:
        print(f"GO classes: {go_decoder.num_classes}")
        print(f"KNN K={artifact['knn_n_neighbors']}, metric={artifact['knn_metric']}")

    # Determine threshold
    threshold = args.threshold
    if threshold is None:
        threshold = artifact.get("best_threshold", 0.5)
    if args.verbose:
    ...
    for seq in SeqIO.parse(args.seq_file, "fasta"):
        ...
        # Get KNN predictions
        y_pred = KNNClassifier.predict_from_artifact(artifact, X_test)[0]
    ...
    if args.use_mlflow:
        ...
        if mlflow is not None:
            params = {
                ...
                "knn_n_neighbors": artifact["knn_n_neighbors"],
                "knn_metric": artifact["knn_metric"],
            }
            ...
            log_inference_results(
                ...
                metrics=metrics,
                tags={
                    "Inference Info": "KNNClassifier ESM-embedding inference",
                    "model_type": "knn",
                },
            )
