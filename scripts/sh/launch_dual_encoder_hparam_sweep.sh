...
for SEED in "${SEEDS[@]}"; do
    for FVH in "${FV_HIDDENS[@]}"; do
        ...
        [ "$FVH" = "32" ] && SHUF=1
        sbatch \
            --export=ALL,SEED=${SEED},FV_HIDDEN=${FVH},SHUFFLE=${SHUF},LEVELS="${LEVELS}",POOL=${POOL},VARIANT=${VARIANT},FEATURE_ALGORITHMS="${FEATURES}" \
            --job-name=dualhp_fvh${FVH}_${POOL}_s${SEED} \
            -o run_dualhp_fvh${FVH}_${POOL}_s${SEED}.out \
            -e run_dualhp_fvh${FVH}_${POOL}_s${SEED}.err \
