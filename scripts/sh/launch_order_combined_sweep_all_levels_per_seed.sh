...
#     VARIANT=hard bash scripts/sh/launch_order_combined_sweep_all_levels_per_seed.sh
#
# The classical feature block defaults to the three PseKRAAC types used in the
# earlier KNN sweep. Override to test other descriptors:
#     FEATURES="PseKRAAC_type_2 PseKRAAC_type_5" bash scripts/sh/launch_order_combined_sweep_all_levels_per_seed.sh
#
# Usage (from the ProtCast repo root on Frontera):
#     bash scripts/sh/launch_order_combined_sweep_all_levels_per_seed.sh
...
for SEED in "${SEEDS[@]}"; do
    sbatch \
        ...
        --job-name=order_comb_${VARIANT}_ml_s${SEED} \
        -o run_order_comb_${VARIANT}_ml_s${SEED}.out \
        -e run_order_comb_${VARIANT}_ml_s${SEED}.err \
        protcastshared/ProtCast/scripts/sh/run_compare_order_combined_multilevel.sh
done
