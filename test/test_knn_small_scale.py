"""test_knn_small_scale.py

Small-scale end-to-end training and inference for the KNNClassifier.
Mirrors test_multilabel_small_scale.py so both models can be tested
with identical data and compared directly.

Usage:

    # Quick test with fake embeddings (no GPU needed)
    python3 test/test_knn_small_scale.py \
        -s test/data/random-level-4_200.fa \
        -v

    # Real ESM-C embeddings
    python3 test/test_knn_small_scale.py \
        -s test/data/random-level-4_200.fa \
        --use_esm \
    ...
    # With MLflow logging and GO term names
    python3 test/test_knn_small_scale.py \
        -s test/data/random-level-4_1800.fa \
        --use_esm \
        --model_type esmc_300m \
    ...
    python3 test/test_knn_small_scale.py \
        -s test/data/random-level-4_1800.fa \
        --use_esm \
        --config_override '{"KNN_N_NEIGHBORS": 20}' \
        -v
"""
...
# ---------------------------------------------------------------------------
# Embedding generation (identical to multilabel small scale test)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# FASTA parsing (identical to multilabel small scale test)
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Small-scale KNNClassifier training and inference"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="ESM-C model type (default: esmc_300m)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to GO OBO file for term names and depth-level metrics"
    )
    ...
    default_config = {
        "USER": "test",
        "EXPERIMENT_NAME": "knn_small_scale",
        "REGISTERED_MODEL_NAME": "knn_test.v0",
    ...
    # --- Step 4: Load GO DAG for depth metrics (optional) ---
    go_dag = None
    if args.obo:
        try:
            from protcast.preprocessing.annotated_godag import AnnotatedGODag
            print(f"\nLoading GO DAG from {args.obo}...")
            go_dag = AnnotatedGODag(args.obo)
            if args.verbose:
                print(f"GO DAG loaded: {len(go_dag.go_terms_map)} terms")
        except Exception as e:
            if args.verbose:
                print(f"Warning: Could not load GO DAG: {e}")

    # --- Step 5: Train KNN ---
    print("\n" + "=" * 60)
    print("STEP 3: Training KNNClassifier")
    print("=" * 60)
    print(f"K={config.get('KNN_N_NEIGHBORS', 5)}, "
          f"metric={config.get('KNN_METRIC', 'cosine')}, "
          f"weights={config.get('KNN_WEIGHTS', 'distance')}")
    ...
    y_val_pred = classifier.predict(classifier.X_val)
    ...
    depth_metrics = classifier.compute_depth_metrics(classifier.y_val, y_val_pred)
    if depth_metrics:
        print("\nFmax by GO term depth:")
        for depth, metrics in sorted(depth_metrics.items()):
            print(f"  Depth {depth}: Fmax={metrics['fmax']:.4f} "
                  f"({metrics['n_terms']} terms, "
                  f"avg {metrics['avg_train_count']:.0f} train samples)")

    # Frequency-bucket breakdown
    freq_metrics = classifier.compute_frequency_metrics(classifier.y_val, y_val_pred)
    if freq_metrics:
        print("\nFmax by training frequency:")
        for bucket, metrics in freq_metrics.items():
            print(f"  {bucket}: Fmax={metrics['fmax']:.4f} "
                  f"({metrics['n_terms']} terms, "
                  f"avg {metrics['avg_train_count']:.0f} train samples)")
    ...
    # Load GO term names from OBO if provided
    go_metadata = {}
    ...
    for pid in demo_pids:
        ...
        y_pred = classifier.predict(embedding)[0]
    ...
    print(f"  Model type:         KNN (K={classifier.knn_n_neighbors})")
    print(f"  Distance metric:    {classifier.knn_metric}")
    print(f"  Weight scheme:      {classifier.knn_weights}")
    ...
    print(f"  Training time:      {round(classifier.training_time, 4)}s")
